...
public class WikiPageRanking extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        boolean isCompleted = runXmlParsing("/wiki", "/user/ramprasadp/poc/wiki/ramprasadp/ranking/iter00");
        if (!isCompleted) return 1;

        String lastResultPath = null;

        for (int runs = 0; runs < 5; runs++) {
            String inPath = "/user/ramprasadp/poc/wiki/ramprasadp/ranking/iter" + nf.format(runs);
            lastResultPath = "/user/ramprasadp/poc/wiki/ramprasadp/ranking/iter" + nf.format(runs + 1);
        ...
        isCompleted = runRankOrdering(lastResultPath, "/user/ramprasadp/poc/wiki/ramprasadp/result");
    ...
    public boolean runXmlParsing(String inputPath, String outputPath) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        Job xmlHakker = new Job(conf, "xmlHakker");
